    ...
    async fetchConversation(id: string): Promise<Conversation> {
        ...
        return createConversation({
            ...
            messages: [
                createMessage({
                    role: ROLES.SYSTEM,
                    content: 'LMSYS conversation capture via internal API is technically restricted due to Gradio architecture. Please use the "Save" feature in the UI if available.',
                    timestamp: Date.now()
                }) as any
            ],
