    ...
    try {
        ...
        extractedMessages.forEach((msg, index) => {
            ...
            uniqueMessages.push(createMessage({
                ...
                metadata: {
                    images: (msg.images || []).filter(img => img && !img.match(/image_generation_content\/\d+$/)),
                    attachments: msg.attachments || []
                }
            }));
        ...
        if (messages.length === 0) {
            messages.push(createMessage({
                id: 'parse-fallback',
                role: ROLES.SYSTEM,
                content: 'Could not parse Gemini conversation structure. Raw data: ' + JSON.stringify(parsedData).slice(0, 1000) + '...',
                timestamp: Date.now()
            }));
        }
        ...
        messages.push(createMessage({
            id: 'parse-error',
            role: ROLES.SYSTEM,
            content: 'Error parsing Gemini conversation: ' + e.message + '\n\nRaw data: ' + JSON.stringify(parsedData).slice(0, 500) + '...',
            timestamp: Date.now()
        }));
    }
...
function determineGeminiRoleImproved(text, index, previousMessages) {
    ...
    if (trimmed.match(/^(Ще генерирам|Ще създам|Ще покажа|Ще направя|I will)/i) && trimmed.length < 200) {
        return ROLES.USER;
    }
