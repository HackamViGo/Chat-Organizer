...
export class TabManager {
  ...
  initialize() {
    ...
    logger.debug('tabs', 'Tab listeners registered')
  }
  ...
  private handleTabUpdated(
    ...
    if (tab.url.includes('gemini.google.com')) {
      if (this.DEBUG_MODE) {
        logger.debug('tabs', 'Gemini detected', tabId)
      }
  ...
  private handleTabActivated(activeInfo: chrome.tabs.TabActiveInfo) {
    ...
    if (this.DEBUG_MODE) {
      logger.debug('tabs', 'Tab activated', activeInfo.tabId)
    }
  }
  ...
  private handleTabRemoved(tabId: number, removeInfo: chrome.tabs.TabRemoveInfo) {
    // Cleanup logic
    if (this.DEBUG_MODE) {
      logger.debug('tabs', 'Tab closed', tabId)
    }
  }
}
