...
export class GrokAdapter extends BasePlatformAdapter {
    ...
    async fetchConversation(conversationId: string, url?: string, payload?: any): Promise<Conversation> {
        return limiters.grok.schedule(async () => {
            // Use scraped data if available (fallback)
            if (payload && payload.messages && payload.messages.length > 0) {
                return {
                    id: conversationId,
                    title: payload.title || 'Grok Conversation',
                    platform: this.platform,
                    messages: payload.messages,
                    url: url || `https://grok.com/c/${conversationId}`,
                    created_at: Date.now(),
                    updated_at: Date.now()
                };
            }
