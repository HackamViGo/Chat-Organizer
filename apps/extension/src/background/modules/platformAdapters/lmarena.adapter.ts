...
export class LMArenaAdapter extends BasePlatformAdapter {
  ...
  async fetchConversation(sessionId: string, url?: string, payload?: any): Promise<Conversation> {
    return limiters.lmarena.schedule(async () => {
      // Use scraped data if available (fallback/primary for arena.ai)
      if (payload && payload.messages && payload.messages.length > 0) {
        return {
          id: sessionId,
          title: payload.title || 'Chatbot Arena Conversation',
          platform: this.platform,
          messages: payload.messages,
          url: url || `https://arena.ai/c/${sessionId}`,
          created_at: Date.now(),
          updated_at: Date.now(),
        }
      }
      ...
      const response = await fetch(`https://chat.lmsys.org/run/predict`, {
        method: 'POST',
        headers: {
  ...
  static extractGradioSession(): { session_hash?: string; fn_index?: string } | null {
    try {
      ...
      logger.error('lmarena', 'Failed to extract Gradio config', e)
    }
    return null
  }
}
