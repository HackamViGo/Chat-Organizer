...
describe('Prompt Injection Tests', () => {
  ...
  beforeEach(() => {
    ...
    // Create instance and register listener
  ...
  describe('Dynamic Menu Updates', () => {
    it('should rebuild menus when prompts change', async () => {
      ...
      vi.spyOn(dynamicMenus, 'rebuildMenus');
      // Trigger storage change listener
      (chrome.storage.onChanged as any)._trigger(mockChanges, 'local');

      expect(dynamicMenus.rebuildMenus).toHaveBeenCalled();
    });

    it('should not rebuild on unrelated storage changes', async () => {
      vi.spyOn(dynamicMenus, 'rebuildMenus');
      const mockChanges = {
        'unrelated_key': { newValue: 'val', oldValue: 'old' }
      };

      (chrome.storage.onChanged as any)._trigger(mockChanges, 'local');

      expect(dynamicMenus.rebuildMenus).not.toHaveBeenCalled();
    });
  });
