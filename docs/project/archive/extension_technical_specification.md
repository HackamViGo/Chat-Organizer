...
│   ├── Config_Global (lib/config-global.js) -> non-module sync
│   ├── DOM_Injector
│   ├── Hover_UI_Manager
...
## FUTURE_VISION (v2.1.0-beta)
### New Popup UI [NON-TOUCHED UNTIL CONFIRMATION]
```
...
|  🤖 GPT (Green) | 💎 GEM (Blue)  | ☁️ CLA (Orange) | 𝕏 GROK (White)  |
|  🌐 PER (Cyan)  | 🏆 LMA (Gold)  | ⚡ DEP (Blue)   | 🧩 QWE (Violet) |
|  🧠 OPEN DASHBOARD (Gradient)                                    |
|  💾 [ Download Selection ]  | Format: [ .md  | v ]               |
```

### MODIFICATIONS_PENDING:
1. **Glassmorphism**: Backdrop blur + semi-transparent dark background.
2. **Dynamic Feedback**: Pulsing "System Ready" status (Green/Red).
3. **Quick Access Board**: Grid 4x2 with model-specific hover shadows.

---

## DEPRECATED_FEATURES:
1. **IMAGES (v2.0.1)**: Feature removed per user request (2026-01-27).
   - Removed `image-saver.js`.
   - Removed `/images` route from dashboard navigation.
   - Disabled access to `/images` in code.
