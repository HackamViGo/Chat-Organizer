...
export async function decryptToken(encryptedValue: unknown): Promise<string | null> {
  if (!encryptedValue) return null

  // If it's not a string, it might be old plain data or a corrupted object
  if (typeof encryptedValue !== 'string') {
    logger.warn('crypto', 'Attempted to decrypt non-string value', typeof encryptedValue)
