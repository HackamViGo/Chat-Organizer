...
export class MessageRouter {
  ...
  constructor(authManager: AuthManager, promptSyncManager: PromptSyncManager, debug = false) {
  ...
  listen() {
    ...
    logger.debug('router', 'Listening for messages...')
  }
  ...
  private handleCreatePrompt(request: MessageRequest, sendResponse: Function): boolean {
    dashboardApi
      .createPrompt(request.promptData)
      .then((data) => sendResponse({ success: true, data }))
      .catch((error) => sendResponse({ success: false, error: error.message }))
    return true
  }
  ...
  private handleStoreGeminiToken(request: MessageRequest, sendResponse: Function): boolean {
    if (request.token) {
      chrome.storage.local.set({ gemini_at_token: request.token }).then(() => {
        if (this.DEBUG_MODE) {
          logger.debug('router', 'Gemini AT token stored')
        }
        sendResponse({ success: true })
      })
    } else {
  ...
  private async injectGeminiScript(tabId?: number) {
    ...
    try {
      ...
      logger.error('router', 'Gemini script injection failed', e)
    }
  }
}
