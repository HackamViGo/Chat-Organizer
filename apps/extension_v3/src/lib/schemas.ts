 ...
 */
export function validateConversation(conversation: unknown): { valid: boolean; error: string | null } {
    if (!conversation || typeof conversation !== 'object') return { valid: false, error: 'Invalid conversation object' };
    ...
    if (!Object.values(PLATFORMS).includes(conv.platform as Platform)) {
        return { valid: false, error: `Invalid platform: ${conv.platform}` };
    }
