...
# BrainBox — Core Rules
**Версия:** 4.0.0 | **Дата:** 2026-02-25 | **Важи за:** Всички агенти и разработчици
...
## 🔴 ПРАВИЛО #0 — Документация (най-важното)

**След всяка промяна по файл — актуализирай документацията.**

Конкретно:
- Промяна в API route → актуализирай `docs/technical/SYNC_PROTOCOL.md`
- Промяна в типове/схеми → актуализирай `docs/technical/DATA_SCHEMA.md`  
- Промяна в архитектура → актуализирай `docs/Mandatory!/ARCHITECTURE.md`
- Промяна в Extension платформа → актуализирай `docs/EXTENSION.md`
- Добавяне на зависимост → актуализирай `docs/technical/MONOREPO_DEPS.md`

**Документите живеят само в `docs/`.  
Единственото изключение: `README.md` файлове в корена на пакети.**

Никакви `.md` файлове в `apps/`, `packages/`, или корена на проекта  
освен `README.md`.
...
- JWT токени в Extension се криптират с AES-GCM.
...
2. **LOG (Detail)** (`docs/agents/logs/{ROLE}_agent.log`): Добави детайлен блок:
   ```text
   Date: YYYY-MM-DD
   Task: [Името от YAML]
   - Списък с конкретни действия (bullet points)
   Status: SUCCESS/FAILED (с описание на грешката, ако е FAILED)
   ```
