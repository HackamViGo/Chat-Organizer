...
;(function () {
  ...
  function setupConversationListObserver() {
    ...
    const attachHoverListeners = () => {
      ...
      conversationLinks.forEach((link) => {
        ...
        const titleSpan = link.querySelector(
          'span.opacity-60, span.truncate span, div.truncate span'
        )
        ...
        // S5-2: { signal } ensures listener is removed when _abortController.abort() is called on SPA navigation
  ...
  function handleSpanHover(spanElement, linkElement, conversationId) {
    ...
    // S5-2: { signal } ensures all listeners are removed when _abortController.abort() is called on SPA navigation
    parentLink.addEventListener('mouseenter', showButtons, { signal: _abortController?.signal })
    parentLink.addEventListener('mouseleave', hideButtons, { signal: _abortController?.signal })

    // Also add to container itself
    container.addEventListener('mouseenter', showButtons, { signal: _abortController?.signal })
    container.addEventListener('mouseleave', hideButtons, { signal: _abortController?.signal })
    ...
    try {
      ...
      if (ui) {
        ...
        if (folderId !== undefined) {
          // null is valid "Uncategorized", undefined is cancel
          handleSave(conversationId, folderId)
        }
      } else {
        // Fallback
